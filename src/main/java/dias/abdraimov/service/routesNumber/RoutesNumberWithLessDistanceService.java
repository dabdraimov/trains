...
public class RoutesNumberWithLessDistanceService implements RoutesNumberWithLessDistance {

    @Override
    public Long routesNumberWithLessDistance(Graph g, City start, City end, Long distance) {
        ...
        while (!q.isEmpty()) {
            ...
            if (current.getStop() < distance) {
                ...
                edges.forEach(edge -> {
                    ...
                    q.add(
                            new CityStop(
                                    edge.getCity(),
                                    current.getStop() + edge.getDistance(),
                                    new Route(newRoute)
                            )
                    );
                });
            ...
            if (current.getStop() < distance && current.getCity().equals(end) && current.getRoute().getCities().size() > 1) {
                result++;
            }
        }
